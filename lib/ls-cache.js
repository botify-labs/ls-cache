...
function supportsStorage() {
  ...
  try {
    ...
    if (exc.name === 'QUOTA_EXCEEDED_ERR' || exc.name === 'NS_ERROR_DOM_QUOTA_REACHED' || exc.name === 'QuotaExceededError') {
      cachedStorage = true;
    } else {
      cachedStorage = false;
    }
...
function Bucket(path) {
   ...
   */
  function handleFullStorageError(key, value) {
    ...
    var targetSize = (value||'').length + key.length + 100;
  ...
  function setItem(key, value) {
    ...
    try {
      ...
      if (e.name === 'QUOTA_EXCEEDED_ERR' || e.name === 'NS_ERROR_DOM_QUOTA_REACHED' || e.name === 'QuotaExceededError') {
        handleFullStorageError(key, value);
        localStorage.setItem(key, value);
      }
      else {
